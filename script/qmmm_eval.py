...
class DeepQMMM (DeepEval) :
    def __init__(self, 
        ...
        if self.has_aparam:
            [self.t_de_da] = tf.gradients (self.t_energy, self.t_aparam)
            print("self.t_de_da = ", self.t_de_da)
    ...
    def eval(self, coords, cells, atom_types,
             fparam = None, aparam = None, atomic = False) :
        ...
        if atomic :
            ...
            e, f, v, de_da = self.eval_inner(coords, cells, atom_types, fparam = fparam, aparam = aparam, atomic = atomic)
            if self.modifier_type is not None:
    ...
    def eval_inner(self,
        ...
        de_da  = self.reverse_map(np.reshape(de_da, [nframes,-1,fdim]), imap)
        ...
        de_da  = np.reshape(de_da, [nframes, natoms, fdim])
        if atomic:
            ae = np.reshape(ae, [nframes, natoms, 1])
            av = np.reshape(av, [nframes, natoms, 9])
            return energy, force, virial, de_da, ae, av
        else :
            return energy, force, virial, de_da